<template>
  <div class="container">
    <modal-grid
      ...
      :data="itemsAdministrativo"
      :access="'administrativos'"
      ...
      <div class="table-responsive">
        <div class="d-flex flex-sm-row flex-column bg-secondary">
          <div class="mr-auto p-2">
            <b-button @click="visibleModal = true" v-if="itemsAdministrativo.length > 0" squared variant="outline-danger" class="mr-2">
              <i class="fa fa-check-square"></i>
              ({{ itemsAdministrativo.length }}) Seleccionados
            </b-button>
        ...
        <kendo-datasource
          ref="data-administrativo"
          :schema-total="'meta.total'"
          :schema-data="'data'"
          :transport-read="{ url: `${url}/administrativos`, beforeSend: readData }"
        ...
        ></kendo-datasource>
        <kendo-grid
          ref="gridAdministrativo"
          :data-source-ref="'data-administrativo'"
          :no-records="true"
        ...
        >
          ...
          ></kendo-grid-column>
          <kendo-grid-column
            v-if="permission('administrativos.update')"
...
export default {
  ...
  computed: {
    ...mapGetters(["itemsAdministrativo"])
  },
  ...
  mounted() {
    let grid = this.$refs.gridAdministrativo.kendoWidget()
    let ds = []
  ...
  methods: {
    reloadTable() {
      this.$refs.gridAdministrativo.kendoWidget().dataSource.filter({})
    },

    update(ev) {
      ev.preventDefault()
      let gridWidget = this.$refs.gridAdministrativo.kendoWidget()
      let tr = $(ev.target).closest('tr')
      let data = gridWidget.dataItem(tr)
      this.$router.push({name: "EditAdministrativo", params: { id: data.id }})
    },

    async deleted() {
      let data = this.itemsAdministrativo.map(item => item.id)
      const response = await AdministrativoService.deleteAdministrativo(data)
      if (response.status === 200) {
        this.$store.dispatch("emptyAdministrativo")
        this.$refs.gridAdministrativo.kendoWidget().dataSource.read()
    ...
    onChange(e) {
      ...
      items.each(function(idx, row) {
        ...
        let index = vm.itemsAdministrativo.findIndex(x => x.id == idValue)
        if (row.className.indexOf("k-state-selected") >= 0) {
          let {parent, dirty, dirtyFields, _events, _handlers, uid,...obj} = grid.dataItem(row)
          vm.$store.dispatch("setItemAdministrativo", obj)
        } else if (index > -1) {
          vm.$store.dispatch("deleteItemAdministrativo", index)
        }
      })
    },
    ...
    dataBound(e) {
      ...
      items.each(function(idx, row) {
        ...
        let index = vm.itemsAdministrativo.some(item => item.id === dataItem)
        if (index) {
