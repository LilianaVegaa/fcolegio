      ...
      <div class="table-responsive">
        <div class="d-flex flex-sm-row flex-column bg-secondary">
          <div class="mr-auto p-2">
            <b-button @click="visibleModal = true" v-if="itemsAsistencia.length > 0" squared variant="outline-danger" class="mr-2">
              <i class="fa fa-check-square"></i>
              ({{ itemsAsistencia.length }}) Seleccionados
            </b-button>
        ...
        <kendo-datasource
          ...
          :schema-data="'data'"
          :transport-read="{ url: `${url}/asistencias`, beforeSend: readData }"
        ...
        <kendo-grid
          ref="gridAsistencia"
          :data-source-ref="'data-asistencia'"
          :no-records="true"
        ...
        >
          ...
          ></kendo-grid-column>
          <kendo-grid-column
            v-if="permission('asistencias.update')"
            :command="[{name: ' ', iconClass: 'fa fa-calendar-check-o', click: onUpdate}]"
            :width="45"
          ></kendo-grid-column>
        </kendo-grid>
      </div>
...
export default {
  ...
  mounted() {
    let grid = this.$refs.gridAsistencia.kendoWidget()
    let ds = []
  ...
  methods: {
    reloadTable() {
      this.$refs.gridAsistencia.kendoWidget().dataSource.filter({})
    },

    onUpdate(ev) {
      ev.preventDefault();
      const gridWidget = this.$refs.gridAsistencia.kendoWidget();
      const tr = $(ev.target).closest('tr');
      const data = gridWidget.dataItem(tr);

      this.$router.push({ name: "CreateAsistencia", params: { id: data.curso_id, gestion: data.gestion_id } });
    },

    async deleted() {
      let data = this.itemsAsistencia.map(item => item.id)
      const response = await AsistenciaService.deleteAsistencia(data)
      if (response.status === 200) {
        this.$store.dispatch("emptyAsistencia")
        this.$refs.gridAsistencia.kendoWidget().dataSource.read()
    ...
    templateName(dataItem) {
      return (
        ...
        kendo.htmlEncode(dataItem.curso) +
        "</span>"
      )
    },
    ...
    onChange(e) {
      ...
      items.each(function(idx, row) {
        ...
        let index = vm.itemsAsistencia.findIndex(x => x.id == idValue)
        if (row.className.indexOf("k-state-selected") >= 0) {
          let {parent, dirty, dirtyFields, _events, _handlers, uid,...obj} = grid.dataItem(row)
          vm.$store.dispatch("setItemAsistencia", obj)
        } else if (index > -1) {
          vm.$store.dispatch("deleteItemAsistencia", index)
        }
      })
    },
    ...
    dataBound(e) {
      ...
      items.each(function(idx, row) {
        ...
        let index = vm.itemsAsistencia.some(item => item.id === dataItem)
        if (index) {
