      ...
      <div class="table-responsive">
        <div class="d-flex flex-sm-row flex-column bg-secondary">
          <div class="mr-auto p-2">
            <b-button @click="visibleModal = true" v-if="itemsCurso.length > 0" squared variant="outline-danger" class="mr-2">
              <i class="fa fa-check-square"></i>
              ({{ itemsCurso.length }}) Seleccionados
            </b-button>
        ...
        <kendo-datasource
          ...
          :schema-data="'data'"
          :transport-read="{ url: `${url}/cursos`, beforeSend: readData }"
...
export default {
  ...
  mounted() {
    let grid = this.$refs.gridCurso.kendoWidget()
    let ds = []
  ...
  methods: {
    reloadTable() {
      this.$refs.gridCurso.kendoWidget().dataSource.filter({})
    },

    update(ev) {
      ev.preventDefault()
      let gridWidget = this.$refs.gridCurso.kendoWidget()
      let tr = $(ev.target).closest('tr')
      let data = gridWidget.dataItem(tr)
      this.$router.push({name: "EditCurso", params: { id: data.id }})
    },

    async deleted() {
      let data = this.itemsCurso.map(item => item.id)
      const response = await CursoService.deleteCurso(data)
      if (response.status === 200) {
        this.$store.dispatch("emptyCurso")
        this.$refs.gridCurso.kendoWidget().dataSource.read()
    ...
    onChange(e) {
      ...
      items.each(function(idx, row) {
        ...
        let index = vm.itemsCurso.findIndex(x => x.id == idValue)
        if (row.className.indexOf("k-state-selected") >= 0) {
          let {parent, dirty, dirtyFields, _events, _handlers, uid,...obj} = grid.dataItem(row)
          vm.$store.dispatch("setItemCurso", obj)
        } else if (index > -1) {
          vm.$store.dispatch("deleteItemCurso", index)
        }
      })
    },
    ...
    dataBound(e) {
      ...
      items.each(function(idx, row) {
        ...
        let index = vm.itemsCurso.some(item => item.id === dataItem)
        if (index) {
