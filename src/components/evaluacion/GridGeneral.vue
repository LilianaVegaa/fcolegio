      ...
      <div class="table-responsive">
        <div class="d-flex flex-sm-row flex-column bg-secondary">
          <div class="mr-auto p-2">
            <b-button @click="visibleModal = true" v-if="itemsEvaluacion.length > 0" squared variant="outline-danger" class="mr-2">
              <i class="fa fa-check-square"></i>
              ({{ itemsEvaluacion.length }}) Seleccionados
            </b-button>
        ...
        <kendo-datasource
          ...
          :schema-data="'data'"
          :transport-read="{ url: `${url}/evaluaciones`, beforeSend: readData }"
        ...
        <kendo-grid
          ref="gridEvaluacion"
          :data-source-ref="'data-evaluacion'"
          :no-records="true"
        ...
        >
          ...
          ></kendo-grid-column>
          <kendo-grid-column
            v-if="permission('evaluaciones.update')"
...
export default {
  ...
  mounted() {
    let grid = this.$refs.gridEvaluacion.kendoWidget()
    let ds = []
  ...
  methods: {
    reloadTable() {
      this.$refs.gridEvaluacion.kendoWidget().dataSource.filter({})
    },

    onUpdate(ev) {
      ev.preventDefault();
      const gridWidget = this.$refs.gridEvaluacion.kendoWidget();
      const tr = $(ev.target).closest('tr');
      const data = gridWidget.dataItem(tr);

      this.$router.push({ name: "CreateEvaluacion", params: { id: data.curso_id, gestion: data.gestion_id } });
    },

    async deleted() {
      let data = this.itemsEvaluacion.map(item => item.id)
      const response = await EvaluacionService.deleteEvaluacion(data)
      if (response.status === 200) {
        this.$store.dispatch("emptyEvaluacion")
        this.$refs.gridEvaluacion.kendoWidget().dataSource.read()
    ...
    onChange(e) {
      ...
      items.each(function(idx, row) {
        ...
        let index = vm.itemsEvaluacion.findIndex(x => x.id == idValue)
        if (row.className.indexOf("k-state-selected") >= 0) {
          let {parent, dirty, dirtyFields, _events, _handlers, uid,...obj} = grid.dataItem(row)
          vm.$store.dispatch("setItemEvaluacion", obj)
        } else if (index > -1) {
          vm.$store.dispatch("deleteItemEvaluacion", index)
        }
      })
    },
    ...
    dataBound(e) {
      ...
      items.each(function(idx, row) {
        ...
        let index = vm.itemsEvaluacion.some(item => item.id === dataItem)
        if (index) {
