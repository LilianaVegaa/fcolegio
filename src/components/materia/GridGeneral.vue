      ...
      <div class="table-responsive">
        <div class="d-flex flex-sm-row flex-column bg-secondary">
          <div class="mr-auto p-2">
            <b-button @click="visibleModal = true" v-if="itemsMateria.length > 0" squared variant="outline-danger" class="mr-2">
              <i class="fa fa-check-square"></i>
              ({{ itemsMateria.length }}) Seleccionados
            </b-button>
        ...
        <kendo-datasource
          ...
          :schema-data="'data'"
          :transport-read="{ url: `${url}/materias`, beforeSend: readData }"
        ...
        >
          ...
          ></kendo-grid-column>
          <kendo-grid-column
            v-if="permission('materias.update')"
...
export default {
  ...
  mounted() {
    let grid = this.$refs.gridMateria.kendoWidget()
    let ds = []
  ...
  methods: {
    reloadTable() {
      this.$refs.gridMateria.kendoWidget().dataSource.filter({})
    },

    update(ev) {
      ev.preventDefault()
      let gridWidget = this.$refs.gridMateria.kendoWidget()
      let tr = $(ev.target).closest('tr')
      let data = gridWidget.dataItem(tr)
      this.$router.push({name: "EditMateria", params: { id: data.id }})
    },

    async deleted() {
      let data = this.itemsMateria.map(item => item.id)
      const response = await MateriaService.deleteMateria(data)
      if (response.status === 200) {
        this.$store.dispatch("emptyMateria")
        this.$refs.gridMateria.kendoWidget().dataSource.read()
    ...
    onChange(e) {
      ...
      items.each(function(idx, row) {
        ...
        let index = vm.itemsMateria.findIndex(x => x.id == idValue)
        if (row.className.indexOf("k-state-selected") >= 0) {
          let {parent, dirty, dirtyFields, _events, _handlers, uid,...obj} = grid.dataItem(row)
          vm.$store.dispatch("setItemMateria", obj)
        } else if (index > -1) {
          vm.$store.dispatch("deleteItemMateria", index)
        }
      })
    },
    ...
    dataBound(e) {
      ...
      items.each(function(idx, row) {
        ...
        let index = vm.itemsMateria.some(item => item.id === dataItem)
        if (index) {
