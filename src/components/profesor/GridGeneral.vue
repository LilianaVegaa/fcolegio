      ...
      <div class="table-responsive">
        <div class="d-flex flex-sm-row flex-column bg-secondary">
          <div class="mr-auto p-2">
            <b-button @click="visibleModal = true" v-if="itemsProfesor.length > 0" squared variant="outline-danger" class="mr-2">
              <i class="fa fa-check-square"></i>
              ({{ itemsProfesor.length }}) Seleccionados
            </b-button>
        ...
        <kendo-datasource
          ...
          :schema-data="'data'"
          :transport-read="{ url: `${url}/profesores`, beforeSend: readData }"
        ...
        >
          ...
          ></kendo-grid-column>
          <kendo-grid-column
            v-if="permission('profesores.update')"
...
export default {
  ...
  mounted() {
    let grid = this.$refs.gridProfesor.kendoWidget()
    let ds = []
  ...
  methods: {
    reloadTable() {
      this.$refs.gridProfesor.kendoWidget().dataSource.filter({})
    },

    update(ev) {
      ev.preventDefault()
      let gridWidget = this.$refs.gridProfesor.kendoWidget()
      let tr = $(ev.target).closest('tr')
      let data = gridWidget.dataItem(tr)
      this.$router.push({name: "EditProfesor", params: { id: data.id }})
    },

    async deleted() {
      let data = this.itemsProfesor.map(item => item.id)
      const response = await ProfesorService.deleteProfesor(data)
      if (response.status === 200) {
        this.$store.dispatch("emptyProfesor")
        this.$refs.gridProfesor.kendoWidget().dataSource.read()
    ...
    onChange(e) {
      ...
      items.each(function(idx, row) {
        ...
        let index = vm.itemsProfesor.findIndex(x => x.id == idValue)
        if (row.className.indexOf("k-state-selected") >= 0) {
          let {parent, dirty, dirtyFields, _events, _handlers, uid,...obj} = grid.dataItem(row)
          vm.$store.dispatch("setItemProfesor", obj)
        } else if (index > -1) {
          vm.$store.dispatch("deleteItemProfesor", index)
        }
      })
    },
    ...
    dataBound(e) {
      ...
      items.each(function(idx, row) {
        ...
        let index = vm.itemsProfesor.some(item => item.id === dataItem)
        if (index) {
