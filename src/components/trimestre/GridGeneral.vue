      ...
      <div class="table-responsive">
        <div class="d-flex flex-sm-row flex-column bg-secondary">
          <div class="mr-auto p-2">
            <b-button @click="visibleModal = true" v-if="itemsTrimetre.length > 0" squared variant="outline-danger" class="mr-2">
              <i class="fa fa-check-square"></i>
              ({{ itemsTrimetre.length }}) Seleccionados
            </b-button>
        ...
        <kendo-datasource
          ...
          :schema-data="'data'"
          :transport-read="{ url: `${url}/trimestres`, beforeSend: readData }"
        ...
        <kendo-grid
          ref="gridTrimestre"
          :data-source-ref="'data-trimestre'"
          :no-records="true"
        ...
        >
          ...
          ></kendo-grid-column>
          <kendo-grid-column
            v-if="permission('trimestres.update')"
...
export default {
  ...
  mounted() {
    let grid = this.$refs.gridTrimestre.kendoWidget()
    let ds = []
  ...
  methods: {
    reloadTable() {
      this.$refs.gridTrimestre.kendoWidget().dataSource.filter({})
    },

    update(ev) {
      ev.preventDefault()
      let gridWidget = this.$refs.gridTrimestre.kendoWidget()
      let tr = $(ev.target).closest('tr')
      let data = gridWidget.dataItem(tr)
      this.$router.push({name: "EditPeriodo", params: { id: data.id }})
    },

    async deleted() {
      let data = this.itemsTrimetre.map(item => item.id)
      const response = await TrimestreService.deleteTrimestre(data)
      if (response.status === 200) {
        this.$store.dispatch("emptyTrimestre")
        this.$refs.gridTrimestre.kendoWidget().dataSource.read()
    ...
    templateName(dataItem) {
      return (
        ...
        kendo.htmlEncode(dataItem.nombre) +
        "</span>"
      )
    },
    ...
    onChange(e) {
      ...
      items.each(function(idx, row) {
        ...
        let index = vm.itemsTrimetre.findIndex(x => x.id == idValue)
        if (row.className.indexOf("k-state-selected") >= 0) {
          let {parent, dirty, dirtyFields, _events, _handlers, uid,...obj} = grid.dataItem(row)
          vm.$store.dispatch("setItemTrimestre", obj)
        } else if (index > -1) {
          vm.$store.dispatch("deleteItemTrimestre", index)
        }
      })
    },
    ...
    dataBound(e) {
      ...
      items.each(function(idx, row) {
        ...
        let index = vm.itemsTrimetre.some(item => item.id === dataItem)
        if (index) {
