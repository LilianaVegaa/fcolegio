      ...
      <div class="table-responsive">
        <div class="d-flex flex-sm-row flex-column bg-secondary">
          <div class="mr-auto p-2">
            <b-button @click="visibleModal = true" v-if="itemsTutor.length > 0" squared variant="outline-danger" class="mr-2">
              <i class="fa fa-check-square"></i>
              ({{ itemsTutor.length }}) Seleccionados
            </b-button>
        ...
        <kendo-datasource
          ...
          :schema-data="'data'"
          :transport-read="{ url: `${url}/tutores`, beforeSend: readData }"
        ...
        >
          ...
          ></kendo-grid-column>
          <kendo-grid-column
            v-if="permission('tutores.update')"
...
export default {
  ...
  mounted() {
    let grid = this.$refs.gridTutor.kendoWidget()
    let ds = []
  ...
  methods: {
    reloadTable() {
      this.$refs.gridTutor.kendoWidget().dataSource.filter({})
    },

    update(ev) {
      ev.preventDefault()
      let gridWidget = this.$refs.gridTutor.kendoWidget()
      let tr = $(ev.target).closest('tr')
      let data = gridWidget.dataItem(tr)
      this.$router.push({name: "EditTutor", params: { id: data.id }})
    },

    async deleted() {
      let data = this.itemsTutor.map(item => item.id)
      const response = await TutorService.deleteTutor(data)
      if (response.status === 200) {
        this.$store.dispatch("emptyTutor")
        this.$refs.gridTutor.kendoWidget().dataSource.read()
    ...
    onChange(e) {
      ...
      items.each(function(idx, row) {
        ...
        let index = vm.itemsTutor.findIndex(x => x.id == idValue)
        if (row.className.indexOf("k-state-selected") >= 0) {
          let {parent, dirty, dirtyFields, _events, _handlers, uid,...obj} = grid.dataItem(row)
          vm.$store.dispatch("setItemTutor", obj)
        } else if (index > -1) {
          vm.$store.dispatch("deleteItemTutor", index)
        }
      })
    },
    ...
    dataBound(e) {
      ...
      items.each(function(idx, row) {
        ...
        let index = vm.itemsTutor.some(item => item.id === dataItem)
        if (index) {
