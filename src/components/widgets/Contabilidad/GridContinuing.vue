  ...
  <div class="container">
    ...
    <vue-custom-scrollbar v-if="items.length > 0" :style="{ width: '100%' }">
      <table>
        <thead>
          <tr>
            <th width="40px">FECHA</th>
            <th width="40px">N° DOC.</th>
            <th width="190px">TÍTULO/RAZÓN SOCIAL</th>
            <th width="240px">GLOSA</th>
            <th width="100px">REFERENCIA</th>
            <th width="10px">T.C.</th>
            <th width="70px">DEBE Bs.</th>
            <th width="70px">HABER Bs.</th>
            <th width="70px">SALDO Bs.</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="(item, index) in items" :key="index">
            ...
            <td>{{ item.type_change | currency }}</td>
...
export default {
  ...
  computed: {
    ...
    totalHaber() {
      return this.items.reduce((acc, item) => acc + Number(item.haber_bs), 0)
    },

    totalSaldo() {
      return this.items.reduce((acc, item) => acc + Number(item.saldo_bs), 0)
    },
  },
