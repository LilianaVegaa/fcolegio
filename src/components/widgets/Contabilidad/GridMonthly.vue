<template>
  <div class="container">
      ...
      <div class="col-sm-12 col-md-10 col-lg-10 col-xl-10">
        ...
        <b-form-group label-for="plan" :invalid-feedback="errors.first('plan')" :state="!errors.has('plan')">
          ...
          <div class="d-table-cell align-middle">
            <b-button @click="getBookHigherMonthly" class="ml-1" size="sm" style="height: 2.5em;">
    ...
    <hr>
    <vue-custom-scrollbar v-if="Object.keys(items).length > 0" :style="{ width: '100%' }">
      <table>
        <thead>
          <tr>
            <th width="130px">MES</th>
            <th width="100px">DEBE Bs</th>
            <th width="100px">HABER Bs</th>
            <th width="100px">SALDO Bs</th>
          </tr>
        </thead>
        <tbody>
      ...
      this.$validator.validate().then(async isTrue => {
        if (isTrue) {
          ...
          try {
            ...
            const request = await ReceiptService.getBookHigherMonthly(data)
            if (request.status === 200) {
