        ...
        <div class="container">
          <div class="row">
            <div class="col-12 col-md-12 col-lg-12 col-xl-12">
              ...
              <b-form-group label-for="title" :invalid-feedback="errors.first('title')" :state="!errors.has('title')">
                <b-form-input 
          ...
          <div class="row">
            ...
            <div class="col-12 col-md-12 col-lg-6 col-xl-6">
              <b-button
                ...
                type="submit" 
                class="float-right" 
              >{{ project.id ? 'ACTUALIZAR' : 'REGISTRAR' }}</b-button>
            </div>
          </div>
        </div>
      </a-spin>
      ...
      <div class="table-responsive">
        <div class="d-flex flex-sm-row flex-column bg-secondary">
          <div class="mr-auto p-2">
            <b-button @click="visibleModal = true" v-if="itemsProject.length > 0" squared variant="outline-danger" class="mr-2">
              <i class="fa fa-check-square"></i>
              ({{ itemsProject.length }}) Seleccionados
            </b-button>
        ...
        <kendo-datasource
          ...
          :schema-data="'data'"
          :transport-read="{ url: `${url}/projects`, beforeSend: readData }"
...
export default {
  ...
  mounted() {
    let grid = this.$refs.gridProject.kendoWidget()
    let ds = []
  ...
  methods: {
    reloadTable() {
      this.$refs.gridProject.kendoWidget().dataSource.filter({})
    },
    ...
    update(ev) {
      ev.preventDefault()
      let gridWidget = this.$refs.gridProject.kendoWidget()
    ...
    async deleted() {
      let data = this.itemsProject.map(item => item.id)
      const response = await ProjectService.deleteProject(data)
      if (response.status === 200) {
        this.$store.dispatch("emptyProject")
        this.$refs.gridProject.kendoWidget().dataSource.read()
    ...
    onChange(e) {
      ...
      items.each(function(idx, row) {
        ...
        let index = vm.itemsProject.findIndex(x => x.id == idValue)
        if (row.className.indexOf("k-state-selected") >= 0) {
          ...
          vm.$store.dispatch("setItemProject", obj)
        } else if (index > -1) {
          vm.$store.dispatch("deleteItemProject", index)
        }
      })
    },
    ...
    dataBound(e) {
      ...
      items.each(function(idx, row) {
        ...
        let index = vm.itemsProject.some(item => item.id === dataItem)
        if (index) {
    ...
    submit: async function() {
      ...
      try {
        if(this.project.id) {
          this._save = await ProjectService.updateProject(this.project.id, this.project)
        } else {
          this._save = await ProjectService.storeProject(this.project)
        }
