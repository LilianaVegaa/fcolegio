          ...
          <div class="row">
            <div class="col-12 col-md-12 col-lg-12 col-xl-12">
              ...
              <b-form-group label-for="direccion" :invalid-feedback="errors.first('direccion')" :state="!errors.has('direccion')">
                <b-form-input
                  v-model="administrativo.direccion"
...
export default {
  ...
  created() {
    if (this.id) {
      this.showAdministrativo()
    }
  },

  methods: {
    showAdministrativo:async function() {
      const response = await AdministrativoService.showAdministrativo(`administrativos/${this.id}/edit`)
    ...
    submit: async function() {
      ...
      try {
        if(vm.id) {
          vm._save = await AdministrativoService.updateAdministrativo(vm.id, vm.administrativo)
        } else {
          vm._save = await AdministrativoService.storeAdministrativo(vm.administrativo)
        }
        if (vm._save.status === 201 || vm._save.status === 200) {
          ...
          if (vm.retry && !this.id) {
            vm.administrativo = new Administrativo()
          } else {
            vm.$router.push({ name: 'ListAdministrativos'})
          }
        }
      } catch (err) {
