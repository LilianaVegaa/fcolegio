...
export default {
  ...
  methods: {
    showCurso:async function() {
      const response = await CursoService.showCurso(`cursos/${this.id}/edit`)
    ...
    submit: async function() {
      ...
      try {
        if(vm.id) {
          vm._save = await CursoService.updateCurso(vm.id, vm.curso)
        } else {
          vm._save = await CursoService.storeCurso(vm.curso)
        }
        if (vm._save.status === 201 || vm._save.status === 200) {
          ...
          if (vm.retry && !this.id) {
            ...
            vm.$router.push({ name: 'ListCursos'})
          }
        }
      } catch (err) {
