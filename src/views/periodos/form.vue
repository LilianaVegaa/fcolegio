<template>
  <fieldset class="col-sm-12 col-md-12 col-lg-12 col-xl-12">
    <legend>Datos Generales:</legend>
    <a-spin :spinning="success">
      <div class="container">
        <div class="d-flex flex-row-reverse bd-highlight">
          <small><strong>Los campos con (*) son obligatorios.</strong></small>
        </div>
        <b-form @submit.stop.prevent="submit">
          <div class="row">
            <div class="col-12 col-md-12 col-lg-4 col-xl-4">
              <strong>
                <label for="nombre">Nombre* :</label>
              </strong>
              <b-form-group label-for="nombre" :invalid-feedback="errors.first('nombre')" :state="!errors.has('nombre')">
                <b-form-input
                  v-model="periodo.nombre"
                  :state="errors.has('nombre') ? false : null"
                  v-validate="'required|max:120'"
                  data-vv-name="nombre"
                  data-vv-as="nombre"
                  type="text"
                ></b-form-input>
              </b-form-group>
            </div>
            <div class="col-12 col-md-12 col-lg-4 col-xl-4">
              <strong>
                <label for="pruebas">Cantidad Evaluaciones * :</label>
              </strong>
              <b-form-group label-for="pruebas" :invalid-feedback="errors.first('pruebas')" :state="!errors.has('pruebas')">
                <b-form-input
                  v-model="periodo.pruebas"
                  :state="errors.has('pruebas') ? false : null"
                  v-validate="'required'"
                  data-vv-name="pruebas"
                  data-vv-as="evaluaciones"
                  type="text"
                ></b-form-input>
              </b-form-group>
            </div>
            <div class="col-12 col-md-12 col-lg-4 col-xl-4">
              <strong>
                <label for="gestion_id">Gestiones * :</label>
              </strong>
              <b-form-group label-for="gestion_id" :invalid-feedback="errors.first('gestion_id')" :state="!errors.has('gestion_id')">
                <v-select
                  label="año"
                  v-model="periodo.gestion_id"
                  :options="gestiones"
                  :reduce="gestion => gestion.id"
                  :state="errors.has('gestion_id') ? false : null"
                  v-validate="'required'"
                  data-vv-name="gestion_id"
                  data-vv-as="gestión"
                ></v-select>
              </b-form-group>
            </div>

          </div>
          <div class="row my-2">
            <div class="col-12 col-md-12 col-lg-6 col-xl-6">
              <b-form-checkbox v-model="retry">
                Quedarme en la página después de registrar los datos.
              </b-form-checkbox>
            </div>
            <div class="col-12 col-md-12 col-lg-6 col-xl-6">
              <b-button
                type="submit"
                class="float-right"
              >{{ id == null ? 'REGISTRAR' : 'ACTUALIZAR' }}</b-button>
            </div>
          </div>
        </b-form>
      </div>
    </a-spin>
  </fieldset>
</template>

<script>
import Periodo from '../../models/Periodo'
import GestionService from '../../services/gestion.service'
import TrimestreService from '../../services/trimestre.service'

export default {
  data() {
    return {
      success: true,
      retry: false,
      id: this.$route.params.id,
      periodo: new Periodo(),
      gestiones: [],
    };
  },

  created() {
    this.listGestiones()
    if (this.id) {
      this.showPeriodo()
    }
  },

  methods: {
    showPeriodo:async function() {
      const response = await TrimestreService.showTrimestre(`trimestres/${this.id}/edit`)
      if (response.status === 200) {
        let obj =  {
          id: response.data.data.id,
          nombre: response.data.data.nombre,
          pruebas: response.data.data.pruebas,
          gestion_id: response.data.data.gestion_id
        }
        this.periodo = obj
        this.success = false
      }
    },

    submit: async function() {
      const vm = this
      vm.success = true
      vm.$validator.errors.clear()
      try {
        if(vm.id) {
          vm._save = await TrimestreService.updateTrimestre(vm.id, vm.periodo)
        } else {
          vm._save = await TrimestreService.storeTrimestre(vm.periodo)
        }
        if (vm._save.status === 201 || vm._save.status === 200) {
          vm.success = false
          this.$emit("success", vm._save.data.data)
          this.$message.success(vm._save.data.message)
          if (vm.retry && !this.id) {
            vm.periodo = new Periodo()
          } else {
            vm.$router.push({ name: 'ListPeriodos'})
          }
        }
      } catch (err) {
        if(err.response.status === 422) vm.$setErrorsFromResponse(err.response.data)
        vm.success = false
      }
    },

    listGestiones: async function() {
      const gestiones = await GestionService.listingGeneral();
      if (gestiones.status === 200) {
        this.gestiones = gestiones.data
        this.success = false
      }
    }
  }
}
</script>
<style scoped>
  >>> .ant-spin-dot-item {
    background-color: #9e0207;
  }

  fieldset {
    background-color: #e9e4e6;
    border-radius: 4px;
  }

  legend {
    background-color: #fff;
    border: 1px solid #ddd;
    border-radius: 4px;
    color: #000;
    font-size: 17px;
    font-weight: bold;
    padding: 3px 5px 3px 7px;
    width: auto;
  }
</style>
