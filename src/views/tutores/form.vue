...
export default {
  ...
  methods: {
    showTutor:async function() {
      const response = await TutorService.showTutor(`tutores/${this.id}/edit`)
    ...
    submit: async function() {
      ...
      try {
        if(vm.id) {
          vm._save = await TutorService.updateTutor(vm.id, vm.tutor)
        } else {
          vm._save = await TutorService.storeTutor(vm.tutor)
        }
        if (vm._save.status === 201 || vm._save.status === 200) {
          ...
          if (vm.retry && !this.id) {
            ...
            vm.$router.push({ name: 'ListTutores'})
          }
        }
      } catch (err) {
